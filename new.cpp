...
int day;int preallotarray[100];int alloted[100];
int fallot[5][3];
int offering[3];
int serving[3];
int reading[2];
...
int main()
{
    ...
    while(preallot>>i)
    {
        preallotarray[sizepreallotarray]=i;sizepreallotarray++;
    }
    todaystimetable();
...
int previousallot(int dev,int s)
{
    ...
    int i,t=1;int x=0;int j;

    for(i=0;i<sizepreallotarray;i=i+2)
    {
        if(preallotarray[i]==dev)
        {
            if(preallotarray[i+1]==0)
                return 1;
            else
                return 0;
        }
